...
keyword_pairs <- user2025 %>%
  mutate(id = row_number()) %>% #id for each session
  separate_rows(keywords, sep = ",") %>% 
  mutate(keywords = str_squish(str_trim(keywords))) %>% #remove whitespaces
  filter(keywords != "", !is.na(session)) %>% #filter empty and missing
  unite(session_key, session, id, remove = FALSE) %>% 
  distinct(session_key, keywords) %>% #remove duplicate
  pairwise_count(item = keywords, feature = session, sort = TRUE, upper = FALSE)
