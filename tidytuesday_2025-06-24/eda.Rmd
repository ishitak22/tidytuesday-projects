...
cases_year %>%
  ...
  ggplot(aes(x = total_cases, y = avg_incidence, label = country)) +
  geom_point(alpha = 0.7, color = "#2c7fb8") +
  geom_text(check_overlap = TRUE, size = 3, hjust = 1.1) +
  scale_x_log10(labels = comma) +
  scale_y_continuous(labels = comma) +
  labs(
    title = "Do Reported Cases Match Incidence Rates?",
    subtitle = "Log-scale reveals anomalies: countries with high incidence but low reported totals",
    x = "Total Measles Cases (log scale)",
    y = "Average Incidence per Million",
    caption = "Countries in top-right = good reporting | Top-left = possibly under-reporting"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold", size = 14),
