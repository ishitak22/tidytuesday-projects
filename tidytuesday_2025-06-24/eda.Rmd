...
## Vaccine Gap Slope Chart (top 15 countries with highest incidence)

To explore the **disconnect between vaccination efforts and real-world outcomes**, we visualise the relationship between each country's **estimated vaccine coverage** and its **reported measles incidence rate**.

- Weak surveillance or under-reporting  
- Gaps in immunisation campaigns or follow-up doses  

```{r slope_data, include=FALSE}
slope_df <- cases_year %>%
  group_by(country) %>%
  summarise(
  ...
  filter(!is.na(mean_incidence) & mean_incidence > 0) %>%
  mutate(
...
```{r long_data, include = false}
slope_df_long <- slope_df %>% pivot_longer(cols = c(vaccine_coverage, mean_incidence),
                              names_to = "metric",
                              values_to = "value")
```

```{r slope_chart, echo=FALSE}
ggplot(slope_df_long, aes(x = metric, y = value, group = country)) +
  geom_line(color = "#1f78b4", size = 1) +
  geom_point(color = "#1f78b4", size = 2) +
  geom_text(
    data = slope_df_long %>% filter(metric == "vaccine_coverage"),
    aes(label = country),
    hjust = 1.1,
    size = 3.5,
    fontface = "italic"
  ) +
  scale_x_discrete(labels = c("Vaccine Coverage (%)", "Incidence per Million")) +
  labs(
    title = "Vaccine Gap: Countries with High Measles Incidence",
    subtitle = "Comparing estimated vaccine coverage vs. actual incidence rates (Top 15 countries)",
    y = NULL,
    x = NULL
  ) +
  theme_minimal(base_size = 12) +
